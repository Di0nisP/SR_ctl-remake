 ...
 * 
 * Удалёнными переменными могут быть: \n
 * \arg выходы - в качестве выдаваемых переменных; \n
 * \arg входы  - в качестве требуемых переменных. \n
 * 
 */
 ...
 * 
 * Объект класса представляет собой удалённый узел, который запрашивает и выдаёт данные,
 * и фактически предоставляет интерфейс к алгоритмам удалённого узла,
 * позволяя представлять их в виде привычных входов и выходов.
 * 
 */
class Link_MPI 
{
...
/**
 * @brief Класс-посредник для обработки списков переменных
 * 
...
protected:
	...
	//+ + + + + + + + + + + + + + + + + + + + + 
	/**
	 * @brief Указатель на список требуемых (удалённых) переменных
	 * 
	 * Переменные в списке ведут себя аналогично локальным выходам, 
	 * только значения формируются удалённми выходами.
	 * 
	 */
