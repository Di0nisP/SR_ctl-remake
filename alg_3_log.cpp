...
//	LIB_EXPORT - это макрос, который указывает, что функция GetCalcClass будет доступна для других модулей
//	Выдаёт указатель на класс, имя файла (INI), по которому можно прочитать уставки
LIB_EXPORT	SR_calc_proc* GetCalcClass(const char* block_name, char* file_name)	
{
	...
	int ext_index = static_cast<int>(strstr(file_name, ".so") - file_name); // Сохранение позиции подстроки ".so" (если таковая найдена)
